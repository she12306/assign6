class Enemy{
  ...
  {
    boolean isCollideWithFighter = (this.x+this.enemyImg.width>=fighter.x)
    &&(fighter.x+fighter.fighterImg.width>=this.x+this.enemyImg.width)
    &&(this.y+this.enemyImg.height>=fighter.y)
    &&(fighter.y+fighter.fighterImg.height>=this.y+this.enemyImg.height);
    return isCollideWithFighter;
  }

  boolean isOutOfBorder()
  {
    boolean isOutOfBorder = (x>width);
